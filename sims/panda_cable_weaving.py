...
from utils.mj import ObjType, RobotInfo, get_pose, load_keyframe, save_keyframe
...
class Gripper(BaseRobot):
    ...
    def get_ee_pose(self) -> sm.SE3:
        return get_pose(self.model, self.data, f"{self.name}/tcp", ObjType.SITE)
...
class MjSim(BaseSim):
    def __init__(self):
        ...
        load_keyframe(self.model, self.data, "unnamed_model/s0_1", self.keyframe_path)
    ...
    def init(self):
        ...
        panda_hand = mjcf.from_path(_XML_PANDA)
    ...
    def get_check_point_frames(self) -> Dict[str, sm.SE3]:
        traj = {}
        traj["init"] = self.gripper.get_ee_pose()

        o_name = self.spec["o0"]["name"]
        o_dir = self.spec["o0"]["dir"]
        z_height = self.gripper.get_ee_pose().t[2]
    ...
    def keyboard_callback(self, key: int):
        ...
        if key is glfw.KEY_PERIOD:
            ...
            save_keyframe(self.model, self.data, "s0_1", self.keyframe_path)
