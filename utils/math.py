...
def dotproduct(v1: Vector, v2: Vector) -> float:
    ...
    ------
    TypeError
        If either v1 or v2 is not a list, tuple, or ndarray of floats.
    ValueError
        If the lengths of v1 and v2 are not equal.
    """
    if not isinstance(v1, (list, tuple, np.ndarray)) or not isinstance(
        v2, (list, tuple, np.ndarray)
    ):
        raise TypeError("Both v1 and v2 must be lists, tuples, or ndarrays of floats.")
    if len(v1) != len(v2):
...
def length(v: Vector) -> float:
    ...
    ------
    TypeError
        If v is not a list, tuple, or ndarray of floats.
    """
    if not isinstance(v, (list, tuple, np.ndarray)):
        raise TypeError("v must be a list, tuple, or ndarray of floats.")
...
def angle(v1: Vector, v2: Vector) -> float:
    ...
    ----------
    v1 : Vector
        The first vector, represented as a list, tuple, or numpy ndarray of floats.
    v2 : Vector
        The second vector, represented as a list, tuple, or numpy ndarray of floats.

    Returns
    -------
    float
        The angle in radians between `v1` and `v2`, computed using the dot product formula.

    Raises
    ------
    TypeError
        If `v1` or `v2` is not a list, tuple, or numpy ndarray of floats.
    ValueError
        If `v1` and `v2` do not have the same length.

    Notes
    -----
    - The angle is calculated using the arccosine of the normalized dot product of the vectors.
    - Ensure that both vectors are non-zero before calling this function.
    """
    if not isinstance(v1, (list, tuple, np.ndarray)) or not isinstance(
        v2, (list, tuple, np.ndarray)
    ):
...
def flip(v: Vector) -> np.ndarray:
    """
    Flip a vector element-wise by multiplying it with -1.
